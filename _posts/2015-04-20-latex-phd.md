...
errors <- function(x,y) paste("$", sprintf("%.2f", x), "\\pm", sprintf("%.2f", y), "$", sep = "")
...
If you are still considering exploring options, then I would think carefully about whether to make your document completely reproducible in this way.
 
Don't get me wrong - I went for the completely reproducible route, and if I was doing it again now I would do the same.
If I was advising my earlier self, I might have said not to bother going fully reproducible, and produced my R code apart from the $\LaTeX$.
...
And for sure, debugging R code in reproducible document is much more challenging.
...
How did this happen?
I didn't manage my dependencies well.
...
## [1] xtable_1.7-4   dplyr_0.4.1    testthat_0.9.1 knitr_1.9     
## 
## loaded via a namespace (and not attached):
##  [1] assertthat_0.1  DBI_0.3.1       evaluate_0.5.5  formatR_1.0    
##  [5] lazyeval_0.1.10 magrittr_1.5    parallel_3.1.3  Rcpp_0.11.4    
...
There are two (well three) very nice solutions to this problem:
...
[Docker](www.docker.io) is something altogether different, and probably represents the future.
...
There are containers ready to go for R - the Docker R project is called 'Rocker', and there are a number of containers available, including the aptly named 'hadleyverse'.
...
Install git, brave the steep learning curve, and you will not regret it.
...
My advice is use it, sooner rather than later!
